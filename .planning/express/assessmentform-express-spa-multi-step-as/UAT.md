...
> Note: Cycles 1-2 resolved infrastructure issues (output: 'standalone' missing from next.config.ts causing Docker build failure; SSL misconfiguration for postgres DB connection). Cycle 3 was a clean run after fixing a port conflict between a native next-server process and the Docker container.
...
**Build fixes applied:**
1. Added `output: 'standalone'` to `next.config.ts` — Dockerfile required Next.js standalone output which was not configured.
2. Added `?sslmode=disable` to `DATABASE_URL` in `docker-compose.yml` and updated `src/lib/db.ts` SSL detection to handle Docker internal hostnames.
