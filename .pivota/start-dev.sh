...
# This project already declares allowedDevOrigins in next.config.ts — no overlay needed.
#
...
# allowedDevOrigins already present in next.config.ts — no overlay needed.
# Skip overlay generation; the project's config handles iframe embedding.

# === DB migrate + seed (after install, before exec) ===
# This is a native Next.js app (not compose-managed) with Drizzle ORM migrations.
# Migrate runs AFTER npm ci (needs node_modules/.bin/tsx) and BEFORE next dev.
# Non-fatal: a transient DB hiccup must not strand boot — but we log loudly
# so the platform + operator can diagnose if the app 5xx's on data routes.
# DATABASE_URL comes from the platform-injected environment or the .env seeded above.
run_migrate() {
  if [[ -n "${DATABASE_URL:-}" ]]; then
    echo "[pivota] running db migrate (drizzle)..."
    if node_modules/.bin/tsx drizzle/migrate.ts 2>&1; then
      echo "[pivota] db migrate: OK"
    else
      echo "[pivota] WARN db migrate failed — app will start but DB-backed routes may error" >&2
    fi
    echo "[pivota] running db seed (drizzle)..."
    if node_modules/.bin/tsx drizzle/seed.ts 2>&1; then
      echo "[pivota] db seed: OK"
    else
      echo "[pivota] WARN db seed failed — continuing" >&2
    fi
  else
    echo "[pivota] WARN DATABASE_URL not set — skipping db migrate + seed; DB-backed routes will error" >&2
  fi
}
...
if [[ -n "$LOCK_FILE_PATH" && -f "$LOCK_FILE_PATH" ]]; then
  ...
  if [[ "$CURRENT_HASH" == "$PREVIOUS_HASH" && "$PRESENCE_OK" == "1" ]]; then
    ...
    # Run migrate + seed after install (node_modules now available)
    run_migrate
  fi
...
# Project declares `dev` as: next dev -H 0.0.0.0 -p 4000
# We defer to npm run dev which respects the project's declared host+port.
