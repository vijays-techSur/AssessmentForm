...
# allowedDevOrigins is already present in next.config.ts — no overlay needed.
# No heavyweight one-time installs required for this stack.
...
# === Post-install: DB migrate + seed (runs AFTER npm ci so tsx is available) ===
# This project uses Drizzle ORM with a PostgreSQL sidecar. Migrate and seed run
# via tsx (a devDependency) — they MUST run after install or tsx is not found.
# Keep non-fatal: a transient DB hiccup must not strand boot.
if [[ -f "drizzle/migrate.ts" ]]; then
  echo "[pivota] running DB migrations (drizzle/migrate.ts)"
  node_modules/.bin/tsx drizzle/migrate.ts 2>&1 || echo "[pivota] WARN db:migrate exited non-zero — app may boot onto an empty schema; check DATABASE_URL and DB health"
fi
if [[ -f "drizzle/seed.ts" ]]; then
  echo "[pivota] running DB seed (drizzle/seed.ts)"
  node_modules/.bin/tsx drizzle/seed.ts 2>&1 || echo "[pivota] WARN db:seed exited non-zero — continuing"
fi
